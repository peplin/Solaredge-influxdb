...
if __name__ == "__main__":
    ...
    print(
        f"Connecting to Solaredge inverter {args.solaredge} on "
        f"port {args.port} using unitid {args.unitid}"
    )
    print(
        f"Writing data to influxDb {args.influxdb} on port "
        f"{args.influxport} every     {args.period} seconds"
    )
    client = ModbusClient(
